...
//		HttpKit.CHARSET = charSet;
	...
	public static char[] postChar(String url, Map<String, String> queryParas, String data, Map<String, String> headers) {
		HttpURLConnection conn = null;
		try {
			conn = getHttpConnection(buildUrlWithQueryString(url, queryParas), POST, headers);
			conn.connect();
	...
	public static char[] postCharArray(String url, String data, Map<String, String> headers){
		return 	 postChar(url, null, data, headers);
	}
...
//					LogKit.error(e.getMessage(), e);
	...
	private static char[] readResponseByte(HttpURLConnection conn)
	{
		char[] arrays=new char[2048*100];
		InputStream inputStream = null;
		try {
			inputStream = conn.getInputStream();
			BufferedReader reader = new BufferedReader(new InputStreamReader(inputStream, CHARSET));
			int m=reader.read(arrays);
			char[] realbytes=new char[m];
		    System.arraycopy(arrays, 0, realbytes, 0, m);
			return realbytes;
		}
		catch (Exception e) {
			throw new RuntimeException(e);
