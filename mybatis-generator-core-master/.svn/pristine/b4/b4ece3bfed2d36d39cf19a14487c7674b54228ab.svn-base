...
public class MyBatisGeneratorConfigurationParser {
    ...
    public Configuration parseConfiguration(Element rootNode)
        ...
        for (int i = 0; i < nodeList.getLength(); i++) {
            ...
            }else if("iocmapper".equals(childNode.getNodeName())) {
            	configuration.setMappconfigdir(childNode.getTextContent());
            }else if("mymapper".equals(childNode.getNodeName())) {
            	configuration.setMybaitmapperdir(childNode.getTextContent());
            }
        }
    ...
    private void parseContext(Configuration configuration, Node node) {
        ...
        for (int i = 0; i < nodeList.getLength(); i++) {
            ...
            }else if("SessionFactory".equals(childNode.getNodeName())){
            	Properties atrr = parseAttributes(childNode);
            	context.SessionFactory=atrr.getProperty("value");
            }
        }
    }
    ...
    private void parseJdbcConnection(Context context, Node node) {
        ...
        String dbName = attributes.getProperty("dbName"); //$NON-NLS-1$
        ...
        if (stringHasValue(dbName)) {
            jdbcConnectionConfiguration.setDbName(dbName);
        }
