...
class MemlessParser {
	...
	private static final String RectTokenE = "//->";
	private static final String EqualTokenE = "//=>";
	private static final String MethodToknE = "//+";
	private String protoPackageName;
	private String outerClassName;
	private String javaPackageName;
	private String protoBuffName;
	...
	void process(String filename) throws Exception {
		int lastx=filename.lastIndexOf("\\")<0?filename.lastIndexOf("/"):filename.lastIndexOf("\\");
		int lastd=filename.lastIndexOf(".");
		this.protoBuffName=filename.substring(lastx+1,lastd);
	...
	private void processInnerMessage(ProtobufMessage parentMessage) throws Exception {
		...
		while ((curToken = getNextIgnoreNewLine()) != null) {
			...
			if (curToken.equals(RectTokenE)) {
				String returnMsg = getNextNotEmpty();
				if(returnMsg.lastIndexOf(".")>0){
					int lastdex=returnMsg.lastIndexOf(".");
					parentMessage.setRecvMsgName(returnMsg.substring(lastdex, returnMsg.length()));
					parentMessage.setRecvPackage(returnMsg.substring(0,lastdex));
					
				}else{
				parentMessage.setRecvPackage(getPackageName());
				parentMessage.setRecvMsgName(returnMsg);
				}
				continue;
			}
			//协议号的标记
			if(curToken.equals(EqualTokenE)){
				String ProtoNumber = getNextNotEmpty();
				parentMessage.setProtoNumber(ProtoNumber);
			}
			//方法标记
			if(curToken.equals(MethodToknE)){
				String Method = getNextNotEmpty();
				parentMessage.setHttpMethod(Method);
			}
			if (curToken.equals(Tokens.MESSAGE)) {
				String messageName = getNextNotEmpty();
				if (messageName == null || !Tokens.isIdentifier(messageName)) {
			...
			if (curToken.equals(Tokens.OPTIONAL_FIELD)) {
				...
				if (name == null || !Tokens.isIdentifier(name)) {
					throw new Exception("Invalid field name: " + name+" type:"+type);
				}
	...
	private void processInnerSquareBraces(ProtobufField fields) throws Exception {
		...
		while ((curToken = getNextIgnoreNewLine()) != null) {
			...
			if (curToken.equals("minval")) {
				consume("=");
				String value = getNextIgnoreNewLine();
				fields.setDefaults(value);
				continue;
			}
			if (curToken.equals("maxval")) {
				consume("=");
				String value = getNextIgnoreNewLine();
				fields.setDefaults(value);
				continue;
			}
			if (curToken.equals("minlen")) {
				consume("=");
				String value = getNextIgnoreNewLine();
				fields.setMinlen(value);
				continue;
			}
			if (curToken.equals("maxlen")) {
				consume("=");
				String value = getNextIgnoreNewLine();
				fields.setMaxlen(value);
				continue;
			}
			if (curToken.equals("regx")) {
				consume("=");
				String value = getNextIgnoreNewLine();
				fields.setStrregx(value);
				continue;
			}
		}
	...
	private static void enrichField(ProtobufField curField, String prefix, String externalPackage, List<ProtobufMessage> allMessages, List<ProtobufEnum> allEnums) throws Exception {
		String beanName = convertNameToJavabean(curField.getName());
		curField.setBeanName(beanName);
		curField.setStreamBeanType(convertNameToJavabean(curField.getType()));
		curField.setJavaFieldName(convertBeanNameToJavaFieldName(curField.getName()));
	...
	static String convertNameToJavabean(String str) {
		...
		for (String curPart : parts) {
			...
			result.append(UNDERSCORE).append(Character.toUpperCase(curPart.charAt(0)));
			if (curPart.length() > 1) {
	...
	private static String loadFile(String filename) throws Exception {
		...
		try {
			...
			while ((curLine = r.readLine()) != null) {
				...
				boolean b =curLine.trim().startsWith("//+");
				boolean b2 =curLine.trim().startsWith("//->");
				boolean b3 =curLine.trim().startsWith("//=>");
